import type { Task, CreateTaskInput, User, UserPreferences, OverviewStats, SchedulingWindow, DailyCapacity, Note, CreateNoteInput, UpdateNoteInput, NoteCategory } from '../utils/types'
...
interface TaskStorage {
  ...
  updatePauseState(taskId: number, isPaused: boolean, pausedTime: number, pauseStartTime: Date | null): Promise<Task>
  ...
  // Notes
  getNotes(params?: { category?: NoteCategory; taskId?: number; search?: string }): Promise<Note[]>
...
class LocalStorageTaskStorage implements TaskStorage {
  ...
  async startFocusSession(taskId: number): Promise<Task> {
    ...
    const tasks = this.getTasksSync()
    const activeFocusTasks = tasks.filter(t => t.isInFocus && t.id !== taskId)
    ...
    for (const task of activeFocusTasks) {
      if (task.focusStartedAt) {
        ...
        await this.updateTask(task.id, { isInFocus: false, focusStartedAt: undefined })
      }
    }
  ...
  async updatePauseState(taskId: number, isPaused: boolean, pausedTime: number, pauseStartTime: Date | null): Promise<Task> {
    return this.updateTask(taskId, {
      isPaused,
      pausedTime,
      pauseStartTime
  ...
  async getDailyCapacity(date?: Date): Promise<DailyCapacity> {
    ...
    const totalEstimated = tasksForToday.reduce((sum, task) => sum + task.estimatedTime, 0)
    const totalActual = tasksForToday.reduce((sum, task) => sum + (task.actualTime || 0), 0)
    const completedCount = tasksForToday.filter(t => t.status === 'completed').length
    ...
    return {
      ...
      utilizationPercent: totalEstimated > 0 ? Math.round((totalActual / totalEstimated) * 100) : 0
    }
  }
  ...
  async getNotes(params?: { category?: NoteCategory; taskId?: number; search?: string }): Promise<Note[]> {
...
class DynamicTaskStorage implements TaskStorage {
  ...
  async updatePauseState(taskId: number, isPaused: boolean, pausedTime: number, pauseStartTime: Date | null): Promise<Task> {
    return this.getStorage().updatePauseState(taskId, isPaused, pausedTime, pauseStartTime)
  }
