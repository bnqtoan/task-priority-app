    ...
    if (search) {
      ...
      filteredNotes = userNotes.filter(
        (note) =>
          ...
          (note.tags &&
            JSON.parse(note.tags as string).some((tag: string) =>
...
notesRouter.put("/:id", zValidator("json", updateNoteSchema), async (c) => {
  ...
  try {
    ...
    if (noteData.tags !== undefined)
      updateData.tags = JSON.stringify(noteData.tags);
    if (noteData.metadata !== undefined)
      updateData.metadata = JSON.stringify(noteData.metadata);
