...
export const getDecisionRecommendation = (task: Task, selectedMethod: string = 'hybrid'): AIRecommendation => {
  ...
  if (selectedMethod === 'simple') {
    ...
    if (impact <= 5 && ease >= 8) {
      return { decision: 'delegate', reason: 'Easy task - delegate to free up time' };
    }
  ...
  if (selectedMethod === 'weighted') {
    ...
    if (score >= 8 && impact >= 7) {
      return { decision: 'do', reason: `High weighted score (${score.toFixed(1)})` };
    }
    if (score >= 7 && ease <= 4) {
  ...
  if (selectedMethod === 'roi') {
    ...
    if (roi >= 1.5 && timeEfficiency >= 5) {
      return { decision: 'do', reason: `High ROI (${roi.toFixed(2)}) & time efficient` };
    }
    if (roi >= 1.5 && ease <= 4) {
      return { decision: 'delegate', reason: `High ROI (${roi.toFixed(2)}) but difficult` };
    }
    if (roi < 1.0 && ease >= 7) {
      return { decision: 'delegate', reason: `Low ROI (${roi.toFixed(2)}), waste of time` };
    }
    if (roi < 0.8) {
  ...
  if (selectedMethod === 'energy') {
    ...
    if (valuePerEnergy >= 1.5) {
      return { decision: 'do', reason: `Excellent energy ROI (${valuePerEnergy.toFixed(2)})` };
    }
    ...
    if (valuePerEnergy < 0.5) {
      return { decision: 'delete', reason: `Poor energy ROI (${valuePerEnergy.toFixed(2)})` };
    }
  ...
  if (selectedMethod === 'hybrid') {
    ...
    if (finalScore >= 3.5 && valueScore >= 6) {
      return { decision: 'do', reason: `Excellent score (${finalScore.toFixed(2)})` };
    }
    ...
    if (finalScore < 2.0) {
      return { decision: 'delete', reason: `Low score (${finalScore.toFixed(2)})` };
    }
